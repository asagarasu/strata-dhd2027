...
from svgkit_71 import LOOM_BOARD, esc, w_of as _w_of
...
def w_of(s, fs, sp=0.0):
    """This board's frozen width metric — svgkit_71.LOOM_BOARD, unchanged.

    The board alone gives the space its own 0.25 width, and svgkit checks that
    branch BEFORE the narrow set — which also contains a space, so the order is
    load-bearing. All of it is baked into the committed SVG; see svgkit_71's
    docstring. The shim keeps the positional third argument the fitting pass
    uses, e.g. w_of(ln, fs, sp).
    """
    return _w_of(s, fs, spacing=sp, **LOOM_BOARD)
